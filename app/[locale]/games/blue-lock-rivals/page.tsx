...
export async function generateMetadata({
  ...
  const path = `/games/blue-lock-rivals`;
...
export default async function BlueLockRivalsGamePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const locale: Locale = isLocale(requestedLocale) ? requestedLocale : defaultLocale;
  const t = getDictionary(locale);
  const lastUpdated = await getPlatformLastUpdated("blue-lock-rivals");
    ...
    <main className="min-h-screen bg-background pt-24">
      ...
      <div className="mx-auto max-w-5xl px-4 pb-24">
        <section className="mt-10 rounded-xl border bg-card p-5 shadow-sm">
          <h2 className="text-2xl font-semibold">Blue Lock Rivals Resources</h2>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-4">
              <h3 className="font-medium text-primary">Redemption Platform</h3>
              <p className="text-sm text-muted-foreground">
                We monitor all active promotional codes. You can find the full list and redemption instructions here:
              </p>
              <Link 
                prefetch={false}
                href={localizePath(locale, "/games/blue-lock-rivals/rewards/redemption-codes")}
              ...
              >
                Blue Lock Rivals Codes &amp; How to Redeem
              </Link>
            </div>
            <div className="space-y-2">
              <h3 className="font-medium">Recent Topics:</h3>
              <ul className="list-disc ml-5 text-muted-foreground text-sm space-y-1">
                <li>Blue Lock Rivals free lucky spins codes 2026</li>
                <li>How to use codes in Blue Lock Rivals</li>
                <li>April 2026 active working codes</li>
                <li>New rewards for Blue Lock Rivals players</li>
              </ul>
            </div>
          </div>
        </section>
