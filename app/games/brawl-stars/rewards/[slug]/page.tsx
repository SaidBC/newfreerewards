...
function getRewardFromConfig(slug: string, locale: Locale) {
  const reward = getLocalizedBrawlStarsRewards(locale).find(
    (item) => item.slug === slug
  );
...
export async function generateStaticParams() {
  return getLocalizedBrawlStarsRewards("en").map((reward) => ({
    slug: reward.slug,
  }));
}
    ...
    <main className="min-h-screen bg-background">
      <section className="mx-auto max-w-5xl px-4 py-16">
        <Button variant={"link"} asChild>
          <Link href={"/games/brawl-stars"} data-trigger-popunder="true">
        ...
        <div className="flex gap-4 items-center">
          <Image
            ...
            src={reward.platform.image || "/images/brawl-stars/logo.jpeg"}
            width={48}
            height={48}
