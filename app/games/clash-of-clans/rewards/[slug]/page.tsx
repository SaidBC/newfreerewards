...
function getRewardFromConfig(slug: string, locale: Locale) {
  const reward = getLocalizedClashOfClansRewards(locale).find(
    (item) => item.slug === slug
  );
...
export async function generateStaticParams() {
  return getLocalizedClashOfClansRewards("en").map((reward) => ({
    slug: reward.slug,
  }));
}
...
function renderBlock(block: RewardContentBlock, index: number) {
  switch (block.type) {
    ...
    case "image":
      const isQrCode = (block.imageAlt || "").toLowerCase().includes("qr") || (block.imageSrc || "").toLowerCase().includes("qr");
      return (
        <div key={index} className={`relative mt-2 overflow-hidden rounded-lg border ${isQrCode ? "max-w-[200px]" : ""}`}>
          <Image
            src={block.imageSrc || "/images/clash-of-clans/Clash_of_Clans.webp"}
    ...
    <main className="min-h-screen bg-background">
      <section className="mx-auto max-w-5xl px-4 py-16">
        <Button variant={"link"} asChild>
          <Link href={"/games/clash-of-clans"} data-trigger-popunder="true">
        ...
        <div className="flex gap-4 items-center">
          <Image
            ...
            src={reward.platform.image || "/images/clash-of-clans/Clash_of_Clans.webp"}
            width={48}
            height={48}
