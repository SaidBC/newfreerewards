...
const PlatformCard = ({
  ...
  <li className="group h-full">
    <Link
      ...
      href={localizePath(
        locale,
        `/${platform.type.toLowerCase()}s/${platform.slug}`,
      )}
    ...
    >
      <div className="relative aspect-video w-full overflow-hidden rounded-xl border">
        <Image
          ...
          src={
            platform.image
              ? storageUrl(platform.image)
              : storageUrl("images/clash-royale/clash-royale.jpg")
          }
          fill
          alt={platform.name}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
          <span className="text-white text-sm font-concert-one uppercase tracking-wider">
            {exploreLabel}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 flex-grow">
        <div className="flex items-center justify-between gap-2">
          <h2 className="font-bold font-concert-one text-xl uppercase tracking-tight">
            {platform.name}
          </h2>
        ...
        <p className="text-muted-foreground text-[10px] uppercase tracking-widest font-medium">
          Last update:{" "}
          {platform.createdAt?.toLocaleDateString(locale, {
            day: "numeric",
            month: "short",
            year: "numeric",
          })}
      ...
      <Button
        size="sm"
        className="w-full font-concert-one uppercase tracking-wider py-5"
      >
        {exploreLabel}
      </Button>
    </Link>
...
export default async function PlatformsListSection({
  ...
  const [gamePlatforms, moneyPlatforms] = await Promise.all([
    ...
    <section id="list" className="py-24 container-wrapper">
      <div className="flex flex-col items-center mb-12 text-center">
        ...
        <p className="max-w-xl text-muted-foreground">
          Find and claim the latest rewards, gifts, and promotions from your
      ...
      <Tabs defaultValue="games" className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md mx-auto h-14 p-1 bg-muted/50 rounded-xl mb-10">
          <TabsTrigger
            value="games"
            className="rounded-lg text-base font-concert-one uppercase transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm"
          >
            {t.home.gamesTab}
          </TabsTrigger>
          <TabsTrigger
