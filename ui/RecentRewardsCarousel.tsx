...
export default function RecentRewardsCarousel({
  ...
  useEffect(() => {
    setVisitedSet(isVisitedForRewards(rewards));
  ...
  function isVisitedForRewards(items: typeof rewards) {
    const set = new Set<string>();
    for (const r of items) {
      if (isVisited(r.platform.slug, r.slug)) {
        set.add(r.id);
      }
    }
    return set;
                ...
                <CardContent className="p-0">
                  <div className="relative aspect-square w-full overflow-hidden bg-muted">
                    ...
                    <div className="absolute top-2 right-2 flex gap-1">
                      {visitedSet.has(reward.id) ? (
                        <span className="bg-primary text-primary-foreground text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter">
                          {t.common.visited}
                        </span>
                      ) : (
                        <span className="bg-primary text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-tighter">
                          New
                        </span>
                      )}
                    </div>
                  ...
                  <div className="p-4 flex flex-col gap-1">
                    ...
                    <h3 className="text-sm font-concert-one uppercase truncate tracking-tight">
                      {reward.title}
                    </h3>
                  </div>
                </CardContent>
